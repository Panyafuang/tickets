  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  constructor(
    ...
    public data: { orderId: string; price: number; email: string }
  ) {
        ...
        if (this.elements) {
          this.cardElement = this.elements.create('card', {
            style: {
              ...
              invalid: {
                // ตรวจสอบว่ามี invalid style ที่ถูกต้อง
                color: '#fa755a',
                iconColor: '#fa755a',
              },
            },
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        ...
        this.snackBar.open(this.cardErrors!, 'Close', {
          panelClass: ['error-snackbar'],
        });
        this.loading = false;
        return;
      ...
      const backendResponse = await this.stripeService
        .createCharge(
          token.id,
          this.orderId
      ...
      if (
        ...
        backendError.error.errors.length > 0
      ) {
